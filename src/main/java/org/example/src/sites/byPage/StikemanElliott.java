...
public class StikemanElliott extends ByPage {
    ...
    protected void accessPage(int index) throws InterruptedException {
        String otherUrl = String.format("https://stikeman.com/en-ca/people#first=%d&sort=%s&numberOfResults=90&f:PositionFacet=[Lawyer]", 90 * index, URLEncoder.encode("@biosortfield ascending", StandardCharsets.UTF_8));
    ...
    protected List<WebElement> getLawyersInPage() {
        By[] webRole = new By[]{By.className("Coveo-biopositionname")};
        String[] validRoles = new String[]{"partner", "counsel", "senior associate"};

        try {
            WebDriverWait wait = new WebDriverWait(this.driver, Duration.ofSeconds(10L));
            List<WebElement> lawyers = (List)wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector("div.DirectoryEntry-detail > div > div")));
    ...
    private String[] getSocials(WebElement lawyer) {
        try {
            String email = this.driver.findElement(By.className("DirectoryEntry-email")).findElement(By.cssSelector("a")).getAttribute("href");
            String phone = this.driver.findElement(By.cssSelector("div:nth-child(3)")).getText();
