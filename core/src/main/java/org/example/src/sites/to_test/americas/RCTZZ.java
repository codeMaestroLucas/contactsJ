...
public class RCTZZ extends ByNewPage {
    ...
    public Object getLawyer(WebElement lawyer) throws Exception {
        String[] info = extractor.extractLawyerText(lawyer, new By[]{By.className("people-name")}, "NAME/ROLE", LawyerExceptions::nameException).split("\n");
        String name = info[0];

        String link = this.openNewTab(lawyer);
        WebElement container = driver.findElement(By.className("data-bottom"));

        String role = info.length > 1 ? info[1] : "Lawyer";
